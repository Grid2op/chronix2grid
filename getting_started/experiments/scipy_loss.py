...
from scipy.sparse import identity, csr_matrix, vstack
...
def adjust_gens(all_loss_orig,
    ...
    rows = []
    cols = []
    for gen_id in range(total_gen):
        for t in range(total_step - 1):
            rows.append(gen_id + t * total_gen)
            cols.append(gen_id + total_gen * (t+1))
            data.append(1.)
            
            rows.append(gen_id + t * total_gen)
            cols.append(gen_id + total_gen * t)
            data.append(-1.)        
    mat_delta_p = csr_matrix((data, (rows, cols)),
    ...
    ramps_and_pmin = np.concatenate((max_ramp_down, p_min))
    ramps_and_pmax = np.concatenate((max_ramp_up, p_max))
    ...
    mat_sum_gen = csr_matrix((data, (rows, cols)),
                             shape=(total_step, cost_matrix.size),
                             dtype=float)
    while True:
        iter_num += 1
        ...
        def target(x):
            return 0.5 * np.sum((x - target_vector)**2)
...
if __name__ == "__main__":
    ...
    path_chronics_fixed = os.path.join(OUTPUT_FOLDER, "fixed_chronics_scipy")
    ...
    fix_loss_multiple_scenarios(env_path,
                                path_chronix2grid,
                                path_chronics_fixed,
                                scenario_ids=[f"{el}_0" for el in li_months][:1],
                                threshold_stop=threshold_stop,
                                max_iter=max_iter)
