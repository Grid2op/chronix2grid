...
if __name__ == "__main__":
    ...
    nb_steps = None
    dt = 5
    path_env = os.path.expanduser("~/data_grid2op/wcci_2022_dev")
    load_seed = 0
    renew_seed = 0
    env_ref_cst = grid2op.make(path_env)
    name_gen = env_ref_cst.name_gen
    gen_type = env_ref_cst.gen_type
    ...
    if error_ is not None:
        # TODO log that !
        raise error_
    final_gen_p = 1.0 * gen_p_after_dispatch
    ## DC data generation
    ################################################ 
    ...
    ### handle_losses -> _fix_losses_one_scenario
    env_for_loss
    scenario_id
    params = loss_param
    env_path = path_env
    env_param = env_for_loss.parameters
    load_df=load_p
    threshold_stop=threshold_stop
    max_iter=max_iter
    max_iter=100
    iter_quality_decrease=20
    percentile_quality_decrease=99
    ...
    ### handle_losses -> _fix_losses_one_scenario -> _adjust_gens
    gen_p = 1.0 * gen_p_orig
    load_p = 1.0 * final_load_p
    load_q = 1.0 * final_load_q
