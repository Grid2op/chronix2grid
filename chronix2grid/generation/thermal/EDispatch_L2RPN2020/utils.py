...
def run_opf(net, demand, gen_max, gen_min, params):
    ...
    # ++  ++  ++  ++
    # Run Linear OPF
    #status, termination_condition = net.lopf(net.snapshots, pyomo=True, solver_name='amplxpress')#pyomo=True, solver_name='amplxpress')
    status, termination_condition = net.lopf(net.snapshots, solver_name='cbc')#pyomo=True, solver_name='amplxpress')
    if status != 'ok':
